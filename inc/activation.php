...
function cocktail_theme_activation_options_init() {
	register_setting(
		'cocktail_activation_options',
		'cocktail_theme_activation_options'
	);
}
add_action('admin_init', 'cocktail_theme_activation_options_init');

function cocktail_activation_options_page_capability() {
	return 'edit_theme_options';
}
add_filter('option_page_capability_cocktail_activation_options', 'cocktail_activation_options_page_capability');

function cocktail_theme_activation_options_add_page() {
	$cocktail_activation_options = cocktail_get_theme_activation_options();

	if (!$cocktail_activation_options) {
		add_theme_page(
			__('Theme Activation', 'cocktail'),
			__('Theme Activation', 'cocktail'),
			'edit_theme_options',
			'theme_activation_options',
			'cocktail_theme_activation_options_render_page'
		);
	} else {
...
add_action('admin_menu', 'cocktail_theme_activation_options_add_page', 50);

function cocktail_get_theme_activation_options() {
	return get_option('cocktail_theme_activation_options');
}

function cocktail_theme_activation_options_render_page() { ?>
	<div class="wrap">
		<h2><?php printf(__('%s Theme Activation', 'cocktail'), wp_get_theme()); ?></h2>
		<div class="updated">
			<?php _e('<p>These settings are optional and should usually be used only on a fresh installation.</p>', 'cocktail'); ?>
		</div>

		<?php settings_errors(); ?>

		<form method="post" action="options.php">
			<?php settings_fields('cocktail_activation_options'); ?>
			<table class="form-table">
				<tr valign="top"><th scope="row"><?php _e('Create a style guide?', 'cocktail'); ?></th>
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php _e('Create a style guide?', 'cocktail'); ?></span></legend>
							<select name="cocktail_theme_activation_options[create_style_guide]" id="create_style_guide">
								<option selected="selected" value="true"><?php echo _e('Yes', 'cocktail'); ?></option>
								<option value="false"><?php echo _e('No', 'cocktail'); ?></option>
							</select>
							<p class="description"><?php printf(__('Create a page called Style Guide to show theme styles', 'cocktail')); ?></p>
						</fieldset>
					</td>
				</tr>
				<tr valign="top"><th scope="row"><?php _e('Change permalink structure?', 'cocktail'); ?></th>
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php _e('Update permalink structure?', 'cocktail'); ?></span></legend>
							<select name="cocktail_theme_activation_options[change_permalink_structure]" id="change_permalink_structure">
								<option selected="selected" value="true"><?php echo _e('Yes', 'cocktail'); ?></option>
								<option value="false"><?php echo _e('No', 'cocktail'); ?></option>
							</select>
							<p class="description"><?php printf(__('Change permalink structure to /&#37;postname&#37;/', 'cocktail')); ?></p>
						</fieldset>
					</td>
				</tr>
				<tr valign="top"><th scope="row"><?php _e('Create navigation menu?', 'cocktail'); ?></th>
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php _e('Create navigation menu?', 'cocktail'); ?></span></legend>
							<select name="cocktail_theme_activation_options[create_navigation_menus]" id="create_navigation_menus">
								<option selected="selected" value="true"><?php echo _e('Yes', 'cocktail'); ?></option>
								<option value="false"><?php echo _e('No', 'cocktail'); ?></option>
							</select>
							<p class="description"><?php printf(__('Create the Primary Navigation menu and set the location', 'cocktail')); ?></p>
						</fieldset>
					</td>
				</tr>
				<tr valign="top"><th scope="row"><?php _e('Add pages to menu?', 'cocktail'); ?></th>
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php _e('Add pages to menu?', 'cocktail'); ?></span></legend>
							<select name="cocktail_theme_activation_options[add_pages_to_primary_navigation]" id="add_pages_to_primary_navigation">
								<option selected="selected" value="true"><?php echo _e('Yes', 'cocktail'); ?></option>
								<option value="false"><?php echo _e('No', 'cocktail'); ?></option>
							</select>
							<p class="description"><?php printf(__('Add all current published pages to the Primary Navigation', 'cocktail')); ?></p>
						</fieldset>
					</td>
				</tr>
			</table>
...
function cocktail_theme_activation_action() {
	if (!($cocktail_theme_activation_options = cocktail_get_theme_activation_options())) {
		return;
	}
	...
	if ($cocktail_theme_activation_options['create_style_guide'] === 'true') {
		$cocktail_theme_activation_options['create_style_guide'] = false;

		$default_pages = array(__('Style Guide', 'cocktail'));
...
Above is an example of a top level header element, or the <code>h1</code> tag. All page titles and ledes have been set in this tag. The Second Level Header tag, <code>h2</code>, has been relegated to any important page level headings.

<hr />

<h2>h2. This is a large header.</h2>
As mentioned above, the Second Level Header has been assigned to important page level headings. Though there is no hard and fast rule regarding the amount of <code>h1</code> tags allowed on a page, but we tend to always only use one and rely on <code>h2</code> for critical page headings.
<h3>h3. This is a medium header.</h3>
The Third Level Header may be used for any form of page level headings which falls below the <code>h2</code> header in a document hierarchy.
<h4>h4. This is a moderate header.</h4>
The Fourth Level Header may be used for any form of page level headings which falls below the <code>h3</code> header in a document hierarchy.
<h5>h5. This is a small header.</h5>
The Fifth Level Header may be used for any form of page level headings which falls below the <code>h4</code> header in a document hierarchy.
<h6>h6. This is a tiny header.</h6>
The Sixth Level Header may be used for any form of page level headings which falls below the <code>h5</code> header in a document hierarchy.
<!--nextpage-->
...
<a class="button expand" href="#">.button.expand</a>
...
http://youtu.be/OTRmyXX6ipU',
		...
		$guide = get_page_by_title(__('Style Guide', 'cocktail'));
	...
	if ($cocktail_theme_activation_options['change_permalink_structure'] === 'true') {
		$cocktail_theme_activation_options['change_permalink_structure'] = false;
	...
	if ($cocktail_theme_activation_options['create_navigation_menus'] === 'true') {
		$cocktail_theme_activation_options['create_navigation_menus'] = false;

		$cocktail_nav_theme_mod = false;

		$primary_nav = wp_get_nav_menu_object(__('Primary Navigation', 'cocktail'));

		if (!$primary_nav) {
			$primary_nav_id = wp_create_nav_menu(__('Primary Navigation', 'cocktail'), array('slug' => 'primary_navigation'));
			$cocktail_nav_theme_mod['primary_navigation'] = $primary_nav_id;
		} else {
			$cocktail_nav_theme_mod['primary_navigation'] = $primary_nav->term_id;
		}

		if ($cocktail_nav_theme_mod) {
			set_theme_mod('nav_menu_locations', $cocktail_nav_theme_mod);
		}
	}

	if ($cocktail_theme_activation_options['add_pages_to_primary_navigation'] === 'true') {
		$cocktail_theme_activation_options['add_pages_to_primary_navigation'] = false;
	...
	update_option('cocktail_theme_activation_options', $cocktail_theme_activation_options);
}
add_action('admin_init','cocktail_theme_activation_action');

function cocktail_deactivation() {
	delete_option('cocktail_theme_activation_options');
}
add_action('switch_theme', 'cocktail_deactivation');
